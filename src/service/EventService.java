...
public class EventService extends Observable {
        ...
        request.addResponseListener((evt) -> {
            ...
            try {
                ...
                for (int i = 0; i < res.size(); i++) {
                    ...
                    Double org_id = (Double) event.get("organisateur_id");
                    e.setOrganisateur_id(org_id.intValue());

                    Double guide_id = (Double) event.get("guide_id");
                    e.setGuide_id(guide_id.intValue());
    ...
    public void participer(Evenement event, User user, String passcode, Observer o) {
        ...
        request.addArgument("paypal", "false");
        request.addResponseListener((evt) -> {
            o.update(this, new String(request.getResponseData()));
        });
    ...
    public void participerPaypal(Evenement event, User user, Observer o) {
        ConnectionRequest request = new ConnectionRequest("http://localhost/tahwissa/web/app_dev.php/service/events/participer");
        request.setPost(false);
        request.addArgument("user_id", user.getId().toString());
        request.addArgument("event_id", event.getId().toString());
        request.addArgument("passcode", "");
        request.addArgument("paypal", "true");
        request.addResponseListener((evt) -> {
            o.update(this, new String(request.getResponseData()));
        });
    ...
    public void aParticipe(Evenement event, User user, Observer o) {

        ConnectionRequest request = new ConnectionRequest("http://localhost/tahwissa/web/app_dev.php/service/events/hasparticipated");
        request.setPost(false);
        request.addArgument("user_id", user.getId().toString());
        request.addArgument("event_id", event.getId().toString());
        request.addResponseListener((evt) -> {
            ...
            if (reply.equals("1")) {
                System.out.println("A participe");
                o.update(this, true);
            } else {
    ...
    public void getByUser(Dialog dlg, Observer o) {
        addObserver(o);
        events = new ArrayList<>();
        ConnectionRequest request = new ConnectionRequest("http://localhost/tahwissa/web/app_dev.php/service/events/findByUser?user_id=" + LoginManager.getUser().getId());
        request.addResponseListener((evt) -> {
            JSONParser parser = new JSONParser();
