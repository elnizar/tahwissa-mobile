...
public class ListPublication {
    ...
    public void start() {
        ...
        hi.getToolbar().addCommandToRightBar("Back", null ,(event) ->{
        ...
        for (int i = 0; i < s.size(); i++) {
            ...
            Image image = URLImage.createToStorage(placeholder, path+"44", "http://localhost/tahwissa/web/" + publication.getImages().get(0).getChemin());
            ...
            Button btnOk = new Button("Lire la Suite ..."); btnOk.setUIID("myToolbar");
...
//        MenuManager.createMenu(hi, theme);
        hi.getToolbar().addCommandToOverflowMenu("Ajouter une Publication", null , (evt) -> {
            AjouterPublication aj = new AjouterPublication();
            aj.init(hi);
            aj.start();
        });
        hi.show();
